  ...
  Stream<ConnectionResult> onConnectivityChanged() {
    ...
    _timer = Timer.periodic(const Duration(seconds: 5), (timer) async {
      ...
      final connected = connectivityResult.any((res) {
        return res == ConnectivityResult.mobile || res == ConnectivityResult.wifi || res == ConnectivityResult.ethernet || res == ConnectivityResult.vpn || res == ConnectivityResult.other;
      });

      if (!connected) {
        _subject.safeAdd(ConnectionResult.disconnected);
        return;
      }
      ...
      try {
        ...
        if (pingRes.statusCode != 204) {
          _subject.safeAdd(ConnectionResult.connectedNoInternet);
        } else {
          _subject.safeAdd(ConnectionResult.connectedWithInternet);
        }
      } catch (_) {
        _subject.safeAdd(ConnectionResult.connectedNoInternet);
      }
